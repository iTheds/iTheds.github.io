...
- **强制刷新**: checkpoint、关库或特定路径会显式调用同步接口
- **检查点机制**: 定期创建检查点，加速恢复过程
- **错误处理**: 完善的异常处理和错误恢复机制

> 说明:当前实现中，事务提交路径通常会先将 COMMIT 记录 flush 到 WAL 文件，但默认并不等价于每次 COMMIT 都执行 `fsync/fdatasync`。若需要更强的提交持久性语义，应结合 `WAL刷盘与提交持久性分析` 一文中的改造建议理解。
...
| 操作 | 典型含义 |
| --- | --- |
| `Flush()` | 将 WAL buffer 写入文件路径 |
| `Sync()` | 在 `Flush()` 之后进一步执行文件级同步 |
