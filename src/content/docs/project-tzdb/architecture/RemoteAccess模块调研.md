---
title: "RemoteAccess模块调研"
description: "远程访问模块技术调研"
---

## 时间说明

该文档保留的是 RemoteAccess 早期调研稿。原稿并未像后续开发稿那样系统标注绝对日期，因此这里不强行补造具体年月日；但调研顺序仍尽量保留为:

1. 先做 ODBC 标准与驱动管理器调研。
2. 再做 Mysql ODBC 调用路径与通信方式调研。
3. 最后回到 RemoteAccess 自身的接口边界与开发准备判断。
...
这一组问题位于前述 ODBC 与 Mysql 调研之后，属于“开始收束到自身实现边界”时留下的判断:
