...
5. 一级结构统一为：
   ...
   - `engineering`
   - `appendix`
6. 除 `raw_snapshot` 外，不保留 `backup/issues/worklogs/test_reports` 等存档型目录，统一整理到上述分类文档目录。
...
10. 所有重组后入口链接必须可达，需同步更新各级 `index.md` / `index.mdx`。
11. 若 `raw_snapshot` 中存在重复内容（如 `issues/`、`worklogs/` 下副本），只整理一次，并在索引层去重。
12. 禁止为了满足索引而创建“占位空文档 / 文档入口页”；只保留有实际内容的整理稿，并同步删除无效索引页。
13. 对“技术日志类”原稿，优先按“日期 + 技术线”拆分为多份日志文档，而不是写成抽象“主题概述”。
14. 技术日志拆分时尽量保留原始描述、代码片段与设计表达，不过度摘要；可以删除明显非技术段落，但不得篡改技术结论。
15. 若总览日志已被完整拆分且不再承载独立信息，应从索引中移除该总览入口，避免重复导航。
...
18. 技术整理时应尽量保留原稿中的时间锚点（如具体日期、阶段时间、版本时间、测试时间）；可以从“连续日志”改成“按专题组织”，但不要把时间信息整体抹平。
19. 若某专题由多个时间片段汇总而成，应在文中显式标注“时间范围”或“时间线索”，而不是只保留抽象结论。
...
6. 修复所有内部相对链接。
7. 补齐各级 `index.md` 导航。
8. 可选保留映射清单；若项目偏好简洁目录，可不单独保留。
9. 最后核对：
   - 无内容丢失
   - 入口可导航
   - 命名一致
   - `backup/issues/worklogs/test_reports` 等冗余目录已从 `raw_snapshot` 外移除
   - `raw_snapshot/` 未改动
   - 无重复文档 / 重复入口（同内容只保留一次整理稿）
   - 无“占位型”空文档（如仅写“后续补充 / 文档入口”）
   - 技术日志文件命名与内容一致，保持 `log` 风格，而非抽象 topic 概述
   - 日志拆分后仍可按日期追溯关键技术决策
